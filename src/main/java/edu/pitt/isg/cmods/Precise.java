package edu.pitt.isg.cmods;/// Contents: Methods for log arithmetic.
/// Author:   John Aronis
/// Date:     March 2017
///
...
public class Precise {
  ...
  public Precise(double x) {
    ...
    else            { System.out.println( x + " " + "ERROR creating Precise number") ; }
  }
  ...
  public static Precise bernoulli(int n, int k, double p) {
    double q = 1.0 - p ;
    if (k>n) return new Precise(0.0) ;
    else if (k==n) return new Precise(1.0) ;
    else return combinations(n,k).multiply(power(p,k)).multiply(power(q,(n-k))) ;
  }
